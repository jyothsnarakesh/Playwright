   ...
   .filter({hasText:'Java'});
    ...
    for(let i=0;i< await matchedRow.count();i++){
        const currentRow=matchedRow.nth(i)
         ...
         const  r1= await currentRow.locator('td').nth(2).textContent();
 const r2=await currentRow.locator('td').nth(3).textContent();
    if(r1=='Java'&& r2=='Advanced'){
       
        const courseName=await currentRow.locator('td').nth(1).textContent();
